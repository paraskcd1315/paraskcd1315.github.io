...
export default function Hero({ startReveal = true }: { startReveal?: boolean }) {
	...
	useEffect(() => {
		...
		const t = setTimeout(() => el.classList.add('go'), HERO_NAME_REVEAL_DELAY_MS);
		...
		const settle = setTimeout(() => el.classList.add('settled'), HERO_NAME_REVEAL_DELAY_MS + 1100 + 460 + 100);
		return () => {
			clearTimeout(t);
			clearTimeout(settle);
		};
	...
	return (
		<section id='hero' className='hero'>
			<a className='hero-badge' href='https://github.com/paraskcd1315' target='_blank' rel='noopener noreferrer' aria-label='KCD'>
				<img src='/kcd-logo.png' alt='KCD' />
			</a>
			<div className={`hero-photo${bgUrl ? ' ready' : ''}`} style={bgUrl ? { backgroundImage: `url(${bgUrl})` } : undefined} />
			<div
				className='hero-bg'
				style={{
